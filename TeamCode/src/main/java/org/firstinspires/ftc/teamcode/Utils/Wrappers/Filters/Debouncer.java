...
public class Debouncer {
    ...
    public Debouncer(double debounceTime, DebounceType type) {
        ...
        if(m_debounceType == DebounceType.kRising) {
            m_baseline = false;
        } else if(m_debounceType == DebounceType.kFalling) {
            m_baseline = true;
        } else {
            m_baseline = false;
        }
    ...
    private void resetTimer() {
        m_prevTimeSeconds = System.currentTimeMillis();
    }

    private boolean hasElapsed() {
        return System.currentTimeMillis() - m_prevTimeSeconds >= m_debounceTimeSeconds;
    }
