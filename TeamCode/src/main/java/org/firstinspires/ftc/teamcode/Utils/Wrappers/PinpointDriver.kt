...
class PinpointDriver(
    ...
    fun getRRPose() : Pose2d {
        return Pose2d(xPosition.toDouble(), yPosition.toDouble(), hOrientation.toDouble())
    }
    fun getPose(): Pose2D {
        return Pose2D(
            DistanceUnit.INCH,
    ...
    fun getVelocityRR(): PoseVelocity2d {
        return PoseVelocity2d(
            Vector2d(xVelocity.toDouble(), yVelocity.toDouble()),
            hVelocity.toDouble()
        )
    }
